...
function NwBuilder(options) {
  var nwjsUrlbase = process.env.npm_config_nwjs_urlbase || process.env.NWJS_URLBASE;
  if (/v$/.test(nwjsUrlbase)) {
    nwjsUrlbase = nwjsUrlbase.replace(/\/v$/, '/');
  }
  var defaults = {
    files: null,
    appName: false,
    ...
    buildDir: "./build",
    cacheDir: "./cache",
    downloadUrl: nwjsUrlbase || "https://dl.nwjs.io/",
